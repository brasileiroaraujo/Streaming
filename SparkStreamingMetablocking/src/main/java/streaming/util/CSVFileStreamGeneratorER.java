 ...
 */
public class CSVFileStreamGeneratorER {
    ...
    public CSVFileStreamGeneratorER(int nFiles, int nRecords, int betweenFilesMsec) {
        _nFiles = nFiles;
        _nRecords = nRecords;
    ...
    private void writeOutput(File f) throws FileNotFoundException {
        ...
        try {
            for (int i = 1; i <= _nRecords; i++) {
              StreamingEntity item = new StreamingEntity(_random, "Key_" + i);
                p.println(item);
            }
        } finally {
            p.close();
